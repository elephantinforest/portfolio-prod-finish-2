...
If the input contains both text and image, use the `gemini-1.5-flash` model.
...
```
>The `gemini-1.5-flash` model (for text-and-image input) is not yet optimized for multi-turn conversations. Make sure to use gemini-pro and text-only input for chat use cases.
